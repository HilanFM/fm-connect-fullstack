...
function buildCounselorFromBody(body) {
  const name = String(body.name || '').trim();
  return {
    id: uid('coun'),
    name,
    image: body.image || '/assets/real-counselor.jpg',
    focus: body.focus || 'Career Guidance',
    languages: Array.isArray(body.languages) ? body.languages : String(body.languages || 'English, Sinhala').split(',').map(x => x.trim()).filter(Boolean),
    bio: body.bio || 'Career counselor profile created by admin.',
    qualification: body.qualification || 'Qualified Career Counselor',
    yearsExperience: body.yearsExperience || body.experience || '5+',
    email: body.email || '',
    phone: body.phone || '',
...
function updateCounselorFromBody(counselor, body) {
  const allowed = ['name','image','focus','bio','qualification','yearsExperience','email','phone','status'];
  allowed.forEach(k => { if (body[k] !== undefined) counselor[k] = body[k]; });
  if (body.languages !== undefined) counselor.languages = Array.isArray(body.languages) ? body.languages : String(body.languages || '').split(',').map(x => x.trim()).filter(Boolean);
  return counselor;
}
function buildSlotFromBody(body) {
  return {
    id: uid('slot'),
    date: body.date || new Date().toISOString().slice(0, 10),
    time: body.time || body.startTime || '10:00',
...
function updateSlotFromBody(slot, body) {
  ['date','time','endTime','mode','meetingLink','location','status','notes'].forEach(k => { if (body[k] !== undefined) slot[k] = body[k]; });
  if (body.capacity !== undefined) slot.capacity = Number(body.capacity || 1);
  return slot;
}
...
async function handleApi(req, res) {
  ...
  try {
    if (method === 'GET' && p === '/api/health') return send(res, 200, { ok: true, app: 'FM Connect Full Stack v9 Counselor Slots' });
    ...
    if (method === 'POST' && p === '/api/counselor/slots') {
      const user = requireUser(req, res, db, ['COUNSELOR']);
      if (!user) return;
      const counselor = db.counselors.find(c => c.id === user.counselorId);
      if (!counselor) return bad(res, 'Counselor profile not found', 404);
      const body = await parseBody(req);
      if (!body.date || !body.time || !body.mode) return bad(res, 'Date, time, and mode are required');
      const slot = buildSlotFromBody(body);
      counselor.slots = counselor.slots || [];
      counselor.slots.unshift(slot);
      writeDb(db);
      return send(res, 201, slot);
    }
    if (method === 'PUT' && p.startsWith('/api/counselor/slots/')) {
      const user = requireUser(req, res, db, ['COUNSELOR']);
      if (!user) return;
      const counselor = db.counselors.find(c => c.id === user.counselorId);
      if (!counselor) return bad(res, 'Counselor profile not found', 404);
      const slotId = p.split('/').pop();
      const slot = (counselor.slots || []).find(s => s.id === slotId);
      if (!slot) return bad(res, 'Slot not found', 404);
      const body = await parseBody(req);
      updateSlotFromBody(slot, body);
      writeDb(db);
      return send(res, 200, slot);
    }
    if (method === 'DELETE' && p.startsWith('/api/counselor/slots/')) {
      const user = requireUser(req, res, db, ['COUNSELOR']);
      if (!user) return;
      const counselor = db.counselors.find(c => c.id === user.counselorId);
      if (!counselor) return bad(res, 'Counselor profile not found', 404);
      const slotId = p.split('/').pop();
      const slot = (counselor.slots || []).find(s => s.id === slotId);
      if (slot && slot.status === 'BOOKED') return bad(res, 'Booked slots cannot be deleted. Disable the slot instead.');
      counselor.slots = (counselor.slots || []).filter(s => s.id !== slotId);
      writeDb(db);
      return send(res, 200, { ok: true });
    }

    if (method === 'POST' && p === '/api/admin/counselors') {
      const admin = requireUser(req, res, db, ['ADMIN']);
      if (!admin) return;
      const body = await parseBody(req);
      if (!body.name || !body.email) return bad(res, 'Counselor name and email are required');
      if (db.users.some(u => u.email === body.email)) return bad(res, 'Email already exists');
      const counselor = buildCounselorFromBody(body);
      db.counselors.unshift(counselor);
      db.users.unshift({ id: uid('u-coun'), name: body.name, email: body.email, phone: body.phone || '', role: 'COUNSELOR', status: body.userStatus || 'ACTIVE', password: body.password || 'demo123', counselorId: counselor.id });
      writeDb(db);
      return send(res, 201, counselor);
    }
    if (method === 'PUT' && p.startsWith('/api/admin/counselors/') && !p.includes('/slots/')) {
      const admin = requireUser(req, res, db, ['ADMIN']);
      if (!admin) return;
      const counselorId = p.split('/').pop();
      const counselor = db.counselors.find(c => c.id === counselorId);
      if (!counselor) return bad(res, 'Counselor not found', 404);
      const body = await parseBody(req);
      updateCounselorFromBody(counselor, body);
      const user = db.users.find(u => u.counselorId === counselor.id);
      if (user) {
        if (body.name !== undefined) user.name = body.name;
        if (body.email !== undefined) user.email = body.email;
        if (body.phone !== undefined) user.phone = body.phone;
        if (body.userStatus !== undefined) user.status = body.userStatus;
        if (body.password) user.password = body.password;
      }
      writeDb(db);
      return send(res, 200, counselor);
    }
    if (method === 'POST' && p.startsWith('/api/admin/counselors/') && p.endsWith('/slots')) {
      const admin = requireUser(req, res, db, ['ADMIN']);
      if (!admin) return;
      const counselorId = p.split('/')[4];
      const counselor = db.counselors.find(c => c.id === counselorId);
      if (!counselor) return bad(res, 'Counselor not found', 404);
      const body = await parseBody(req);
      if (!body.date || !body.time || !body.mode) return bad(res, 'Date, time, and mode are required');
    ...
    if (method === 'DELETE' && p.startsWith('/api/admin/counselors/') && p.includes('/slots/')) {
      const admin = requireUser(req, res, db, ['ADMIN']);
      if (!admin) return;
      const parts = p.split('/');
      const counselorId = parts[4];
...
server.listen(PORT, () => {
  console.log(`FM Connect full-stack v10 course filters running at http://localhost:${PORT}`);
});
...
// SPA fallback (CRITICAL FIX FOR HOSTING)
app.get("*", (req,res)=>{
  res.sendFile(path.join(__dirname,"public","index.html"));
});

// port fix for hosting platforms
const PORT = process.env.PORT || 3000;

app.listen(PORT, ()=>{
  console.log("FM Connect running on port", PORT);
});
