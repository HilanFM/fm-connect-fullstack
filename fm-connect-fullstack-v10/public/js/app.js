...
// ===== CLEAN V12 RESTORE FIX (STABLE SCROLL) =====
...
// patch existing renderPage if exists
const _renderPage = window.renderPage;
window.renderPage = function(route) {
  if (_renderPage) _renderPage(route);
  safeScrollTop();
};

// fallback hooks
window.addEventListener("load", safeScrollTop);
window.addEventListener("popstate", safeScrollTop);
...
// ===== V16 CONTAINER SCROLL SAAS ARCHITECTURE =====
...
function navigate(path) {
  ...
  if (typeof renderPage === "function") renderPage(path);
