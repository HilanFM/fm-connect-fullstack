...
function footer() {
  return `<footer class="footer">
    ...
    <div class="container footer-grid">
      ...
      <div><h4>Explore</h4><a href="#institutes">Institutes</a><a href="#explore">Courses</a><a href="#explore">Degrees</a><a href="#videos">Videos</a><a href="#articles">Resources</a></div>
...
async function homePage() {
  ...
  return `<section class="home-hero" style="--hero-image:url('${esc(banner.image)}')">
    ...
    <div class="container home-hero-inner">
      ...
      <form id="searchForm" class="hero-search-card">
        <div class="search-tabs"><span class="active">🎓 Courses</span><span>🏫 Institutes</span></div>
        <div class="hero-search-grid"><input name="q" value="${esc(state.query)}" placeholder="Search courses, degrees or keywords..."/><select name="category">${categories.map(c => `<option ${state.category === c ? 'selected' : ''}>${esc(c)}</option>`).join('')}</select><select><option>All Locations</option><option>Colombo</option><option>Kandy</option><option>Online</option></select><select><option>All Study Levels</option><option>Degree</option><option>Diploma</option><option>Certificate</option></select><button class="btn btn-primary">Search</button></div>
      </form>
    </div>
  </section>
  ...
  <section class="section pt-0"><div class="container">${sectionHead('Featured Degrees & Courses','Sponsored courses, degrees, certificates, free courses, and online options.','<a href="#explore" class="text-link">View all courses →</a>')}<div class="course-tabs"><span class="active">All</span><span>Degree</span><span>Diploma</span><span>Certificate</span><span>Free Course</span><span>Online</span></div><div class="grid grid-4">${topCourses.map(courseCard).join('')}</div></div></section>
...
async function explorePage() {
  ...
  return `<section class="page-hero small"><div class="container"><p class="eyebrow">Course Discovery</p><h1>Explore degrees, certificates, diplomas and free courses</h1><p class="muted">Search by course, institute, category, cost, duration, or keyword.</p><form id="searchForm" class="filter-bar"><input name="q" value="${esc(state.query)}" placeholder="Search courses, degrees or keywords..."/><select name="category">${categories.map(c => `<option ${state.category === c ? 'selected' : ''}>${esc(c)}</option>`).join('')}</select><button class="btn btn-primary">Search</button></form></div></section><section class="section"><div class="container"><div class="grid grid-3">${courses.map(courseCard).join('') || '<p>No courses found.</p>'}</div></div></section>`;
}
...
async function institutePage(slug) {
  ...
  const gallery = i.gallery && i.gallery.length ? i.gallery : [i.image, '/assets/gallery-expo-students.png', '/assets/counselor-career-guidance.png'];
  return `<section class="institute-profile-hero" style="--hero-image:url('${esc(i.image)}')"><div class="container profile-hero-content"><div class="profile-logo-card"><div class="logo-text">${esc(i.shortName || 'FM')}</div></div><div><p class="eyebrow">${esc(i.category)} • ${i.accountType === 'SPONSORED' ? 'Featured Partner' : 'Partner Institute'}</p><h1>${esc(i.name)}</h1><div class="rating-line">${stars(i.rating || 4.7)} <span>${esc(i.rating || 4.7)} rating • Sri Lanka • Established education partner</span></div><div class="button-row"><a class="btn btn-primary" href="#explore">Apply Now</a><a class="btn btn-light" href="#counseling">Book Counseling</a><a class="btn btn-outline translucent" href="#contact">♡ Enquire</a></div></div></div></section>
  <section class="profile-metrics"><div class="container profile-metrics-grid">${smallMetric('Students', '12,500+', '👥')}${smallMetric('Programs', String(i.courses?.length || i.courses || 0), '🎓')}${smallMetric('Free Courses', String(i.freeCoursesCount || 0), '✨')}${smallMetric('Placement Focus', '98%', '📈')}</div></section>
  <section class="profile-tabs"><div class="container"><span class="active">Overview</span><span>Degrees & Courses</span><span>Gallery</span><span>Videos</span><span>Admissions</span><span>Reviews</span><span>Contact</span></div></section>
  <section class="section"><div class="container detail-grid"><div><div class="card form-card profile-overview"><h3>About ${esc(i.shortName || i.name)}</h3><p>${esc(i.longDescription || i.description)}</p><div class="overview-columns"><div><h4>Facilities</h4><ul class="feature-list">${listItems(i.facilities || ['Modern labs','Career services','Student support'])}</ul></div><div><h4>Popular Fields</h4><div class="chips">${['Engineering','Business','Computing','Design','Sciences'].map(x => `<span class="chip">${x}</span>`).join('')}</div><h4>Scholarships</h4><p class="muted">Merit-based and need-based options available.</p></div></div></div><div class="mt-2">${sectionHead('Gallery','','<a class="text-link" href="#institutes">View all photos →</a>')}<div class="gallery-strip">${gallery.map(g => `<img src="${esc(g)}" alt="${esc(i.name)} gallery"/>`).join('')}</div></div><div class="mt-2">${sectionHead('Videos','','<a class="text-link" href="#videos">View all videos →</a>')}<div class="grid grid-2">${instVideos.map(videoCard).join('') || '<p>No videos yet.</p>'}</div></div></div><aside class="sidebar-stack"><div class="card form-card"><h3>Contact & Inquiry</h3><p><b>Phone:</b> ${esc(i.phone || '+94 71 545 0000')}</p><p><b>Email:</b> ${esc(i.email || 'info@futureminds.lk')}</p><p><b>Website:</b> ${esc(i.website || '#')}</p><a class="btn btn-primary" href="#contact">Submit Inquiry</a></div><div class="card form-card"><h3>Next Open Day</h3><div class="open-day"><strong>24</strong><span>May 2026</span></div><p>Campus tour, program information, and counseling.</p><a class="btn btn-outline" href="#contact">Register Now</a></div><div class="card form-card"><h3>Upcoming Intake</h3><p><b>July 2026 Intake</b></p><p class="muted">Applications open.</p><a class="btn btn-primary" href="#explore">Apply Now</a></div></aside></div></section>
  <section class="section section-soft"><div class="container">${sectionHead('Degrees & Courses','Programs offered by this institute.','<a href="#explore" class="text-link">View all courses →</a>')}<div class="grid grid-2">${(i.courses || []).map(courseCard).join('') || '<p>No published courses yet.</p>'}</div></div></section>`;
}
async function coursePage(slug) {
  ...
  <section class="profile-tabs"><div class="container"><span class="active">About</span><span>Modules</span><span>Videos</span><span>Recommendations</span><span>Testimonials</span><span>Reviews</span></div></section>
  <section class="section"><div class="container detail-grid"><div><div class="card form-card"><h3>What you’ll learn</h3><ul class="feature-list">${listItems(c.skills || ['Career-ready knowledge','Practical exposure','Professional guidance'])}</ul></div><div class="card form-card mt-2"><h3>There are ${esc((c.modules || []).length || 4)} modules in this course</h3>${(c.modules || []).map((m, idx) => `<div class="module-card"><span>Module ${idx+1}</span><h4>${esc(m)}</h4><p class="muted">Includes videos, readings, quizzes, and practice activities.</p></div>`).join('')}</div></div><aside class="sidebar-stack"><div class="card form-card"><h3>Course summary</h3><p><b>Institute:</b> ${esc(c.institute)}</p><p><b>Award:</b> ${esc(c.award || c.type)}</p><p><b>Starts:</b> ${esc(c.startDate || 'Upcoming intake')}</p><p><b>Discount:</b> ${esc(c.discount || 'Contact institute')}</p><p><b>Entry:</b> ${esc(c.entryRequirements || 'Contact institute')}</p><button class="btn btn-primary" data-lead="ENROLLMENT_REQUEST" data-course="${esc(c.id)}">Enroll Now</button></div><div class="card form-card"><h3>Related careers</h3><div class="chips">${(c.relatedJobs || []).map(job => `<span class="chip">${esc(job)}</span>`).join('')}</div></div></aside></div></section>
  <section class="section section-soft"><div class="container">${sectionHead('Course / Degree Videos','Course explainers, student experience videos, module previews, and related guidance.','<a class="text-link" href="#videos">View all videos →</a>')}<div class="grid grid-4">${courseVideos.map(videoCard).join('') || '<p>No videos yet.</p>'}</div></div></section>`;
}
...
function counselorDash(d) {
  const tabs = [{key:'overview',label:'Overview'}, {key:'stats',label:'Stats'}, {key:'videos',label:'My Videos'}];
  let body = '';
  if (state.tab === 'stats') body = `<div class="grid grid-3">${stat('Confirmed', String(d.stats?.confirmedBookings || 0), 'Current confirmed sessions', '✅')}${stat('Completed', String(d.stats?.completedBookings || 0), 'Completed sessions', '🏁')}${stat('Open Slots', String((d.counselor?.slots || []).filter(s => s.status === 'AVAILABLE').length), 'Available counseling slots', '📅')}${stat('Videos', String((d.videos || []).length), 'Counselor guidance videos', '▶')}</div>`;
...
function adminDash(d) {
  const tabs = [{key:'overview',label:'Overview'}, {key:'stats',label:'Stats'}, {key:'institutes',label:'Institutes'}, {key:'users',label:'Users'}, {key:'courses',label:'Courses'}, {key:'inquiries',label:'Contact Us'}, {key:'videos',label:'Videos'}, {key:'requests',label:'Institute Requests'}, {key:'content',label:'Content'}];
  let body = '';
...
async function bindForms() {
  document.getElementById('searchForm')?.addEventListener('submit', e => { e.preventDefault(); const f = new FormData(e.target); state.query = f.get('q') || ''; state.category = f.get('category') || 'All'; if ((route()[0] || 'home') !== 'explore') nav('explore'); else render(); });
