...
function serveStatic(req, res) {
  ...
  fs.readFile(abs, (err, data) => {
    ...
    const types = { '.html':'text/html', '.css':'text/css', '.js':'application/javascript', '.svg':'image/svg+xml', '.png':'image/png', '.jpg':'image/jpeg', '.jpeg':'image/jpeg', '.webp':'image/webp' };
...
async function saveImage(req, res, db, roles) {
  ...
  if (!match) return bad(res, 'Only base64 image data URLs are supported');
  const ext = match[1].split('/')[1].replace('svg+xml', 'svg').replace(/[^a-z0-9]/gi, '');
  const fileName = `${uid('img')}.${ext || 'png'}`;
...
async function handleApi(req, res) {
  ...
  try {
    if (method === 'GET' && p === '/api/health') return send(res, 200, { ok: true, app: 'FM Connect Full Stack v2' });
    ...
    if (method === 'GET' && p === '/api/dashboard') {
      const user = requireUser(req, res, db);
      if (!user) return;
      if (user.role === 'STUDENT') return send(res, 200, { user: safeUser(user), leads: db.leads.filter(l => l.studentId === user.id), bookings: db.bookings.filter(b => b.studentId === user.id), notifications: db.notifications.filter(n => n.userId === user.id), recommended: sortCourses(db.courses.filter(c => c.status === 'PUBLISHED')).slice(0, 3) });
      if (user.role === 'INSTITUTE_OWNER') return send(res, 200, { user: safeUser(user), institute: db.institutes.find(i => i.id === user.instituteId), courses: sortCourses(db.courses.filter(c => c.instituteId === user.instituteId)), leads: db.leads.filter(l => l.instituteId === user.instituteId) });
      if (user.role === 'COUNSELOR') return send(res, 200, { user: safeUser(user), counselor: db.counselors.find(c => c.id === user.counselorId), bookings: db.bookings.filter(b => b.counselorId === user.counselorId) });
      if (user.role === 'ADMIN') return send(res, 200, { users: db.users.map(safeUser), institutes: db.institutes, courses: db.courses, counselors: db.counselors, articles: db.articles, feedback: db.feedback, pendingArticles: db.articles.filter(a => a.status === 'PENDING_APPROVAL'), pendingFeedback: db.feedback.filter(f => f.status === 'PENDING_APPROVAL'), instituteRequests: db.instituteRequests, leads: db.leads, bookings: db.bookings, banners: db.banners });
    }

    if (method === 'POST' && p === '/api/shared/image') return saveImage(req, res, db, ['ADMIN','INSTITUTE_OWNER']);
    if (method === 'POST' && p === '/api/institute/image') return saveImage(req, res, db, ['ADMIN','INSTITUTE_OWNER']);
...
server.listen(PORT, () => {
  console.log(`FM Connect full-stack v3 image-upgraded system running at http://localhost:${PORT}`);
});
