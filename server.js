...
async function handleApi(req, res) {
  ...
  try {
    if (method === 'GET' && p === '/api/health') return send(res, 200, { ok: true, app: 'FM Connect Full Stack v4' });
    ...
    if (method === 'GET' && p === '/api/dashboard') {
      ...
      if (user.role === 'COUNSELOR') return send(res, 200, { user: safeUser(user), counselor: db.counselors.find(c => c.id === user.counselorId), bookings: db.bookings.filter(b => b.counselorId === user.counselorId), videos: db.videos.filter(v => v.category === 'Counseling' && v.status === 'PUBLISHED'), stats: { confirmedBookings: db.bookings.filter(b => b.counselorId === user.counselorId && b.status === 'CONFIRMED').length, completedBookings: db.bookings.filter(b => b.counselorId === user.counselorId && b.status === 'COMPLETED').length } });
    ...
    if (method === 'POST' && p === '/api/admin/videos') {
      ...
      const body = await parseBody(req);
      if (!body.title || !body.videoUrl) return bad(res, 'Title and video URL are required');
      const video = { id: uid('vid'), title: body.title, category: body.category || 'Platform', scope: body.scope || 'PUBLIC', ownerType: body.ownerType || 'PLATFORM', ownerId: body.ownerId || 'platform', instituteId: body.instituteId || '', thumbnail: body.thumbnail || '/assets/video-platform-tour.png', description: body.description || '', videoUrl: body.videoUrl, status: body.status || 'PUBLISHED', featured: Boolean(body.featured), views: 0, createdAt: new Date().toISOString() };
      db.videos.unshift(video);
      writeDb(db);
      return send(res, 201, video);
    }
    if (method === 'PUT' && p.startsWith('/api/admin/videos/')) {
      ...
      if (!video) return bad(res, 'Video not found', 404);
      ['title','category','scope','ownerType','ownerId','instituteId','thumbnail','description','videoUrl','status'].forEach(k => { if (body[k] !== undefined) video[k] = body[k]; });
      if (body.featured !== undefined) video.featured = Boolean(body.featured);
      writeDb(db);
      return send(res, 200, video);
    }
...
server.listen(PORT, () => {
  console.log(`FM Connect full-stack v4 amendment system running at http://localhost:${PORT}`);
});
