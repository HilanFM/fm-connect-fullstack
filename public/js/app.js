...
function footer() {
  return `<footer class="footer"><div class="container footer-grid"><div><h4>FM Connect</h4><p class="muted">Education marketplace, career guidance, institute management, contact inquiries, video content, and admin operations demo.</p></div><div><h4>Explore</h4><a href="#explore">Courses</a><a href="#institutes">Institutes</a><a href="#videos">Videos</a><a href="#counseling">Counseling</a></div><div><h4>Connect</h4><a href="#contact">Contact Us</a><a href="#contact">Reach Us</a><a href="#institute-register">Become a Partner</a></div><div><h4>Demo</h4><p class="muted">Full-stack starter with JSON database.</p></div></div><div class="footer-bottom">© 2026 FM Connect</div></footer>`;
}
...
function videoCard(v, manageButtons = '') {
  return `<article class="card video-card">${img(v.thumbnail, v.title)}<div class="card-body"><div class="card-tags">${pill(v.category || 'Video','blue')}${v.featured ? pill('Featured','amber') : ''}${statusPill(v.status || 'PUBLISHED')}</div><h3>${esc(v.title)}</h3><p>${esc(v.description || '')}</p><p class="muted">Views: ${esc(v.views || 0)}</p><div class="card-footer"><a class="btn btn-primary" href="${esc(v.videoUrl || '#')}" target="_blank" rel="noreferrer">Watch Video</a>${manageButtons}</div></div></article>`;
}
  ...
  return `
    <section class="section contact-section">
      <div class="container detail-grid">
        <div>
          ${sectionHead('Contact Us / Reach Us', 'Students, parents, institutes, and partners can send inquiries directly to the FM Connect team.')}
          <div class="grid grid-2">
            ${stat('Phone', contact.phone || '+94 71 545 0000', 'Call or WhatsApp', '☎')}
            ${stat('Email', contact.email || 'info@futureminds.lk', 'General inquiries', '✉')}
            ${stat('Office Hours', contact.hours || 'Weekdays', 'Support availability', '⏰')}
            ${stat('Location', contact.address || 'Colombo, Sri Lanka', 'Reach us for events and partnerships', '📍')}
          </div>
        </div>
        <div class="card form-card">
          ...
          <form id="inquiryForm" class="form-grid">
            ...
            <label>Subject<input name="subject" required placeholder="Course / Institute / Partnership inquiry" /></label>
...
async function homePage() {
  ...
  return `<section class="hero-section"><div class="container hero-grid"><div>${pill('Full-stack v4 amendments','blue')}<h1>Build the skills for the future, today.</h1><p>Explore institutes, degrees, certificates, free courses, counseling, contact inquiries, video content, and admin-managed platform records.</p><div class="hero-actions"><a class="btn btn-primary" href="#explore">Explore Courses</a><a class="btn btn-outline" href="#contact">Contact Us</a><a class="btn btn-outline" href="#institute-register">Register Institute</a></div></div><div class="hero-visual">${img(banner.image, 'Future Minds banner', '')}</div></div></section><section class="section"><div class="container grid grid-3">${stat('Courses', String(data.courses.length), 'Published courses', '🎓')}${stat('Institutes', String(data.institutes.length), 'Active institutes', '🏫')}${stat('Videos', String(videos.length), 'Published video content', '▶')}</div></section><section class="section section-soft"><div class="container">${sectionHead('Featured courses','Sponsored and high engagement courses appear first.','<a href="#explore" class="text-link">View all</a>')}<div class="grid grid-3">${topCourses.map(courseCard).join('')}</div></div></section><section class="section"><div class="container">${sectionHead('Video highlights','Use videos for product tours, institute promotions, and student guidance.','<a href="#videos" class="text-link">View videos</a>')}<div class="grid grid-3">${videos.map(videoCard).join('')}</div></div></section><section class="section section-soft"><div class="container">${sectionHead('Partner institutes','Institutes can be created by admin and managed by institute owners.')}<div class="grid grid-2">${topInstitutes.map(instituteCard).join('')}</div></div></section>${inquirySection(data.contactSettings || {})}`;
}
...
async function institutePage(slug) {
  ...
  const instVideos = allVideos.filter(v => v.instituteId === i.id || v.ownerId === i.id || v.ownerType === 'PLATFORM').slice(0, 3);
  return `<section class="detail-hero">${img(i.image, i.name, 'detail-cover')}<div class="container detail-copy"><div class="card surface-overlay"><div class="card-tags">${statusPill(i.status)}${pill(i.accountType,'amber')}</div><h1>${esc(i.name)}</h1><p>${esc(i.longDescription || i.description)}</p><div class="button-row"><a class="btn btn-primary" href="#explore">Explore Courses</a><a class="btn btn-outline" href="#contact">Inquiry</a><a class="btn btn-outline" href="${esc(i.website)}" target="_blank">Website</a></div></div></div></section><section class="section"><div class="container detail-grid"><div><div class="card form-card"><h3>About</h3><p>${esc(i.description)}</p><div class="chips">${(i.facilities || []).map(f => `<span class="chip">${esc(f)}</span>`).join('')}</div></div><div class="card form-card mt-2"><h3>Featured free courses</h3>${(i.featuredFreeCourses || []).map(c => `<p>✓ ${esc(c)}</p>`).join('') || '<p>No free courses listed.</p>'}</div></div><div class="card form-card"><h3>Reach this institute</h3><p><b>Email:</b> ${esc(i.email)}</p><p><b>Phone:</b> ${esc(i.phone)}</p><p><b>WhatsApp:</b> ${esc(i.whatsapp)}</p><a class="btn btn-primary" href="#contact">Send Inquiry</a></div></div></section><section class="section section-soft"><div class="container">${sectionHead('Institute videos','Promo videos and guidance videos related to this institute.')}<div class="grid grid-3">${instVideos.map(videoCard).join('') || '<p>No videos yet.</p>'}</div></div></section><section class="section"><div class="container"><h2>Courses from ${esc(i.shortName)}</h2><div class="grid grid-3">${(i.courses || []).map(courseCard).join('') || '<p>No published courses yet.</p>'}</div></div></section>`;
}
async function coursePage(slug) {
  const c = await api(`/api/public/courses/${encodeURIComponent(slug)}`);
  const allVideos = await api('/api/public/videos');
  const courseVideos = allVideos.filter(v => (c.videos || []).includes(v.id) || v.category === c.category || v.ownerType === 'PLATFORM').slice(0, 3);
  return `<section class="detail-hero">${img(c.image, c.title, 'detail-cover')}<div class="container detail-copy"><div class="card surface-overlay"><div class="card-tags">${c.sponsored ? pill('Sponsored','amber') : ''}${statusPill(c.status)}${c.isFree ? pill('Free Course','green') : pill(c.type,'blue')}</div><h1>${esc(c.title)}</h1><p>${esc(c.overview || c.description)}</p><div class="meta-grid meta-grid-4"><div><span class="meta-label">Institute</span><strong>${esc(c.institute)}</strong></div><div><span class="meta-label">Duration</span><strong>${esc(c.duration)}</strong></div><div><span class="meta-label">Mode</span><strong>${esc(c.studyMode)}</strong></div><div><span class="meta-label">Fee</span><strong>${esc(money(c.price,c.currency))}</strong></div></div><div class="button-row"><button class="btn btn-primary" data-lead="ENROLLMENT_REQUEST" data-course="${esc(c.id)}">Enroll Now</button><button class="btn btn-outline" data-lead="INTERESTED" data-course="${esc(c.id)}">Interested</button><a class="btn btn-outline" href="#contact">Ask a Question</a></div></div></div></section><section class="section"><div class="container detail-grid"><div><div class="card form-card"><h3>Course description</h3><p>${esc(c.description)}</p></div><div class="card form-card mt-2"><h3>Modules</h3>${(c.modules || []).map(m => `<p>✓ ${esc(m)}</p>`).join('')}</div></div><div><div class="card form-card"><h3>Details</h3><p><b>Award:</b> ${esc(c.award)}</p><p><b>Start:</b> ${esc(c.startDate)}</p><p><b>Discount:</b> ${esc(c.discount)}</p><p><b>Entry:</b> ${esc(c.entryRequirements)}</p></div><div class="card form-card mt-2"><h3>Skills</h3><div class="chips">${(c.skills || []).map(s => `<span class="chip">${esc(s)}</span>`).join('')}</div></div></div></div></section><section class="section section-soft"><div class="container">${sectionHead('Course videos','Use videos to explain program value, modules, career outcomes, and student experience.')}<div class="grid grid-3">${courseVideos.map(videoCard).join('') || '<p>No videos yet.</p>'}</div></div></section>`;
}
...
async function videosPage() {
  ...
  return `<section class="page-hero small"><div class="container"><h1>Videos</h1><p class="muted">Platform tours, institute promotions, course explainers, and counseling guidance videos.</p></div></section><section class="section"><div class="container grid grid-3">${items.map(videoCard).join('') || '<p>No videos published.</p>'}</div></section>`;
}
async function contactPage() {
  const contact = await api('/api/public/contact');
  return `<section class="page-hero small"><div class="container"><h1>Contact Us / Reach Us / Inquiry</h1><p class="muted">Send questions about institutes, courses, counseling, sponsorships, or partnerships.</p></div></section>${inquirySection(contact)}`;
}
function loginPage() {
...
function instituteDash(d) {
  const tabs = [{key:'overview',label:'Overview'}, {key:'stats',label:'Stats'}, {key:'profile',label:'Institute Profile'}, {key:'courses',label:'Courses / Degrees'}, {key:'videos',label:'Videos'}, {key:'leads',label:'Leads'}];
  let body = '';
  if (state.tab === 'profile') body = instituteProfileForm(d.institute);
  else if (state.tab === 'courses') body = instituteCoursesPanel(d.courses);
  else if (state.tab === 'videos') body = instituteVideosPanel(d.videos || []);
...
function videoFields(v = {}) {
  return `<label>Title<input name="title" value="${esc(v.title || '')}" required/></label><label>Category<input name="category" value="${esc(v.category || '')}"/></label><label>Status<select name="status">${['PUBLISHED','DRAFT','DISABLED'].map(x => `<option ${v.status === x ? 'selected' : ''}>${x}</option>`).join('')}</select></label><label>Video URL<input name="videoUrl" value="${esc(v.videoUrl || '')}" required/></label><label>Thumbnail path<input name="thumbnail" value="${esc(v.thumbnail || '/assets/video-platform-tour.png')}"/></label><label class="full">Description<textarea name="description">${esc(v.description || '')}</textarea></label><label>Featured<input type="checkbox" name="featured" ${v.featured ? 'checked' : ''}/></label>`;
}
function instituteVideosPanel(videos) {
  return `<div class="grid grid-2"><div class="card form-card"><h3>Add institute video</h3><form id="instituteVideoCreateForm" class="form-grid">${videoFields({ status:'PUBLISHED', category:'Institute', thumbnail:'/assets/video-institute-onboarding.png' })}<button class="btn btn-primary">Create video</button></form></div><div class="stack">${videos.map(v => `<details class="card form-card"><summary><b>${esc(v.title)}</b> ${statusPill(v.status)}</summary><form class="instituteVideoEditForm form-grid mt-2" data-id="${esc(v.id)}">${videoFields(v)}<button class="btn btn-primary">Save video</button><button type="button" class="btn btn-outline" data-delete-institute-video="${esc(v.id)}">Delete video</button></form></details>`).join('') || '<p>No videos yet.</p>'}</div></div>`;
}
function counselorDash(d) {
  const tabs = [{key:'overview',label:'Overview'}, {key:'stats',label:'Stats'}, {key:'videos',label:'Guidance Videos'}];
  let body = '';
  if (state.tab === 'stats') body = `<div class="grid grid-3">${stat('Confirmed', String(d.stats?.confirmedBookings || 0), 'Current confirmed sessions', '✅')}${stat('Completed', String(d.stats?.completedBookings || 0), 'Completed sessions', '🏁')}${stat('Open Slots', String((d.counselor?.slots || []).filter(s => s.status === 'AVAILABLE').length), 'Available counseling slots', '📅')}</div>`;
  else if (state.tab === 'videos') body = `<div class="grid grid-3">${(d.videos || []).map(videoCard).join('') || '<p>No guidance videos.</p>'}</div>`;
  else body = `<div class="grid grid-3">${stat('Bookings', String(d.bookings.length), 'Student sessions', '💬')}${stat('Open slots', String((d.counselor?.slots || []).filter(s => s.status === 'AVAILABLE').length), 'Available', '📅')}${stat('Focus', d.counselor?.focus || 'Career Guidance', 'Specialization', '🎯')}</div><div class="card form-card mt-2"><h3>Bookings</h3>${d.bookings.map(b => `<p><b>${esc(b.studentName)}</b> • ${esc(b.date)} ${esc(b.time)} • ${esc(b.status)}</p>`).join('') || '<p>No bookings yet.</p>'}</div>`;
...
function adminDash(d) {
  const tabs = [{key:'overview',label:'Overview'}, {key:'stats',label:'Stats'}, {key:'institutes',label:'Institutes'}, {key:'users',label:'Users'}, {key:'courses',label:'Courses'}, {key:'inquiries',label:'Inquiries'}, {key:'videos',label:'Videos'}, {key:'requests',label:'Institute Requests'}, {key:'content',label:'Content'}];
  let body = '';
  ...
  else if (state.tab === 'stats') body = `<div class="grid grid-3">${stat('Active Students', String(d.stats?.activeStudents || 0), 'Student accounts', '👥')}${stat('Active Institutes', String(d.stats?.activeInstitutes || 0), 'Live institute profiles', '🏫')}${stat('Published Courses', String(d.stats?.publishedCourses || 0), 'Visible courses', '🎓')}${stat('New Inquiries', String(d.stats?.newInquiries || 0), 'Need follow-up', '📩')}${stat('Videos', String(d.stats?.totalVideos || 0), 'Managed videos', '▶')}${stat('Total Leads', String(d.leads.length), 'Student course requests', '📈')}</div>`;
  else body = `<div class="grid grid-3">${stat('Users', String(d.users.length), 'All role users', '👥')}${stat('Institutes', String(d.institutes.length), 'Active and blocked', '🏫')}${stat('Courses', String(d.courses.length), 'All statuses', '🎓')}</div><div class="grid grid-3 mt-2">${stat('Inquiries', String((d.inquiries || []).filter(i => i.status === 'NEW').length), 'New contact requests', '📩')}${stat('Videos', String((d.videos || []).length), 'Platform and institute videos', '▶')}${stat('Leads', String(d.leads.length), 'Student requests', '📨')}</div>`;
...
function adminInquiriesPanel(d) {
  return `<div class="card form-card"><h3>Contact / Reach Us / Inquiry submissions</h3><div class="stack">${(d.inquiries || []).map(i => `<div class="card form-card"><div class="card-tags">${statusPill(i.status)}</div><h3>${esc(i.subject)}</h3><p><b>${esc(i.name)}</b> • ${esc(i.email)} • ${esc(i.phone || '')}</p><p>${esc(i.message)}</p><p class="muted">${esc(i.createdAt || '')}</p>${i.status !== 'RESOLVED' ? `<button class="btn btn-primary btn-small" data-resolve-inquiry="${esc(i.id)}">Mark Resolved</button>` : ''}</div>`).join('') || '<p>No inquiries yet.</p>'}</div></div>`;
}
function adminVideosPanel(d) {
  return `<div class="grid grid-2"><div class="card form-card"><h3>Add platform video</h3><form id="adminVideoCreateForm" class="form-grid">${videoFields({ status:'PUBLISHED', category:'Platform', thumbnail:'/assets/video-platform-tour.png' })}<label>Institute owner optional<select name="instituteId"><option value="">Platform video</option>${instituteOptions(d.institutes)}</select></label><button class="btn btn-primary">Create video</button></form></div><div class="stack">${(d.videos || []).map(v => `<details class="card form-card"><summary><b>${esc(v.title)}</b> ${statusPill(v.status)} <span class="muted">${esc(v.category)}</span></summary><form class="adminVideoEditForm form-grid mt-2" data-id="${esc(v.id)}">${videoFields(v)}<label>Institute owner optional<select name="instituteId"><option value="">Platform video</option>${instituteOptions(d.institutes, v.instituteId || '')}</select></label><button class="btn btn-primary">Save video</button><button type="button" class="btn btn-outline" data-delete-admin-video="${esc(v.id)}">Delete video</button></form></details>`).join('') || '<p>No videos yet.</p>'}</div></div>`;
}
...
async function bindForms() {
  ...
  document.getElementById('inquiryForm')?.addEventListener('submit', async e => { e.preventDefault(); try { await api('/api/public/inquiries', { method:'POST', body: JSON.stringify(formToObject(e.target)) }); toast('Inquiry submitted successfully'); e.target.reset(); } catch(err) { toast(err.message); } });
  ...
  document.getElementById('instProfileForm')?.addEventListener('submit', async e => { e.preventDefault(); try { const obj = formToObject(e.target); const imgPath = await uploadFromForm(e.target); if (imgPath) obj.image = imgPath; obj.featuredFreeCourses = String(obj.featuredFreeCourses || '').split(',').map(x => x.trim()).filter(Boolean); obj.facilities = String(obj.facilities || '').split(',').map(x => x.trim()).filter(Boolean); await api('/api/institute/profile', { method:'PUT', body: JSON.stringify(obj) }); toast('Institute profile saved'); render(); } catch(err) { toast(err.message); } });
  ...
  document.getElementById('instituteVideoCreateForm')?.addEventListener('submit', async e => { e.preventDefault(); try { await api('/api/institute/videos', { method:'POST', body: JSON.stringify(formToObject(e.target)) }); toast('Institute video created'); render(); } catch(err) { toast(err.message); } });
  document.querySelectorAll('.instituteVideoEditForm').forEach(form => form.addEventListener('submit', async e => { e.preventDefault(); try { await api(`/api/institute/videos/${form.dataset.id}`, { method:'PUT', body: JSON.stringify(formToObject(form)) }); toast('Video updated'); render(); } catch(err) { toast(err.message); } }));
  ...
  document.getElementById('adminVideoCreateForm')?.addEventListener('submit', async e => { e.preventDefault(); try { const obj = formToObject(e.target); if (obj.instituteId) { obj.ownerType = 'INSTITUTE'; obj.ownerId = obj.instituteId; obj.scope = 'INSTITUTE'; } await api('/api/admin/videos', { method:'POST', body: JSON.stringify(obj) }); toast('Video created'); render(); } catch(err) { toast(err.message); } });
  document.querySelectorAll('.adminVideoEditForm').forEach(form => form.addEventListener('submit', async e => { e.preventDefault(); try { const obj = formToObject(form); if (obj.instituteId) { obj.ownerType = 'INSTITUTE'; obj.ownerId = obj.instituteId; obj.scope = 'INSTITUTE'; } await api(`/api/admin/videos/${form.dataset.id}`, { method:'PUT', body: JSON.stringify(obj) }); toast('Video updated'); render(); } catch(err) { toast(err.message); } }));
}
function bindActions() {
  ...
  document.querySelectorAll('[data-resolve-inquiry]').forEach(b => b.addEventListener('click', async () => { await api(`/api/admin/inquiries/${b.dataset.resolveInquiry}/resolve`, { method:'POST' }); toast('Inquiry resolved'); render(); }));
}
